...
def write_asc(in_array, asc_path, xll, yll, c_size, nodata=-37267):
    """
...
def blur_mean(in_array, s_nodata, radius):
    ...
    with warnings.catch_warnings():
        ...
        circular_mean = convolve_fft(nan_array, kernel, nan_treatment='interpolate')  #, normalize_kernel=False)
...
def hillshade(in_array, azimuth, altitude, nodata, cell_size, scale=False):
    """
    Custom implementation of hillshading, using the algorithm from the source
    code for gdaldem. The inputs and outputs are the same as in gdal or ArcGIS.
    in_array:       The input array, should be read using the supper_array
                    technique from below.
    azimuth:        The sun's azimuth, in degrees.
    ...
    # Create new array with nodata values set to np.nan (for edges)
    ...
    azimuth_radians = azimuth * np.pi / 180.
    ...
    sin_altitude = np.sin(altitude_radians)
    cos_azimuth = np.cos(azimuth_radians)
    cos_altitude = np.cos(altitude_radians)
    sin_azimuth = np.sin(azimuth_radians)
    xx_plus_yy = x * x + y * y
    alpha = y * cos_azimuth * cos_altitude - x * sin_azimuth * cos_altitude
    x = None
    y = None
    shaded = (sin_altitude - alpha) / np.sqrt(1 + xx_plus_yy)
    # result is +-1, scale to 0-255, multiply by weight
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', category=RuntimeWarning)
        # newmax = 255
        # newmin = 0
...
def skymodel(in_array, lum_lines, overlap, nodata, resolution):
    """
    Creates a unique hillshade based on a skymodel, implementing the method
    ...
    for line in lum_lines:
        azimuth = float(line[0])
        altitude = float(line[1])
        weight = float(line[2])
        ...
        shadowed = shadows(in_array, azimuth, altitude, resolution, overlap, nodata * 5)
        # shade = hillshade(nan_array, azimuth=azimuth, altitude=altitude, scale=False) * weight
        # shadowed = shadowing.shadows(nan_array, azimuth, altitude, cell_size, overlap, nodata)
...
def shadows(in_array, azimuth, altitude, resolution, overlap, nodata):
    ...
    azimuth = 90. - azimuth  # convert from 0 = north, cw to 0 = east, ccw

    azimuth_radians = azimuth * np.pi / 180.
    altitude_radians = altitude * np.pi / 180.
    delta_j = math.cos(azimuth_radians)
    delta_i = -1. * math.sin(azimuth_radians)
    tangent_altitude = math.tan(altitude_radians)
    ...
    distances = []
    for step_number in range(1, max_steps):
        distance = step_number * resolution
        step_height = distance * tangent_altitude
        i_distance = delta_i * step_number
        j_distance = delta_j * step_number
                ...
                if in_bounds and in_height:
                    ...
                    if next_elev > point_elev and next_elev > critical_height:
                        ...
                        for step2 in range(1, step):
                            shadowed_i = int(round(i + distances[step2 - 1][1]))
                            shadowed_j = int(round(j + distances[step2 - 1][2]))
                            shadow_array[shadowed_i, shadowed_j] = 0
...
def mdenoise(in_array, s_nodata, cell_size, threshold, normal, vertex, tile=None, verbose=False):
    """
    Smooths an array of elevations using the mesh denoise algorithm by Sun et
    al (2007), Fast and Effective Feature-Preserving Mesh Denoising
    (http://www.cs.cf.ac.uk/meshfiltering/index_files/Page342.htm).
    in_array:       The input array, should be read using the supper_array
                    technique from below.
    threshold:      Threshold parameter for mdenoise.exe; range [0,1]
    normal:         Normal updating iterations for mdenoise; try between 10
                    and 50. Larger values increase smoothing effect and runtime
    vertex:         Vertex updating iterations for mdenoise; try between 10
    ...
    write_asc(in_array, temp_s_path, 1, 1, cell_size, s_nodata)

    # Call mdenoise on temporary file
    args = (
        settings.MDENOISE_PATH, '-i', temp_s_path, '-t', str(threshold), '-n', str(normal), '-v', str(vertex), '-o',
        temp_t_path
    )
    mdenoise_output = subprocess.check_output(args, shell=False, universal_newlines=True)
    if verbose:
